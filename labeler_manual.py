...
def main():
    ...
    # Getting label ID - Check to prevent IndexError
    ...
    try:
        ...
        response.raise_for_status()  # Will raise an error for HTTP response codes 4xx/5xx
        
        data = response.json()
        print(f"** Data is **: {data}")

        # Safely check if 'objects' and 'dynamic_criteria' are non-empty before accessing
        if data.get('objects') and len(data['objects']) > 0:
            dynamic_criteria = data['objects'][0].get('dynamic_criteria', [])
            if dynamic_criteria:
                label_id = dynamic_criteria[0].get('label_id')
                if label_id:
                    print(f"Label ID: {label_id}")
                else:
                    print("Error: label_id not found in dynamic_criteria.")
            else:
                print("Error: No dynamic_criteria found.")
        else:
            print("Error: No objects found in response.")
        
    except requests.exceptions.RequestException as e:
        print(f"Error during label request: {e}")
    except json.JSONDecodeError:
        print("Error decoding the JSON response from the label request.")
    
    if 'label_id' not in locals():  # Ensure label_id is defined before continuing
        return

    # Now use the label ID in a PUT request to update the label
    url = f"https://{ip_address}/api/v4.0/labels/{label_id}"
    ...
    try:
        ...
        response.raise_for_status()  # Will raise an error for HTTP response codes 4xx/5xx
        print("Label update response:", response.text)
    except requests.exceptions.RequestException as e:
        print(f"Error during label update request: {e}")
    except json.JSONDecodeError:
        print("Error decoding the JSON response from the label update request.")
